...
import { ShoppingCart, Check, Flame, Star, ChevronRight } from 'lucide-react';
...
export default function Menu() {
  ...
  const handleCategoryClick = (categoryId: string) => {
    ...
    if (element) {
      const offset = 120; // sticky header + category nav offset
  ...
  useEffect(() => {
    const handleIntersection = (entries: IntersectionObserverEntry[]) => {
      // Find the entry that has the largest visible area
      const visibleEntries = entries.filter((entry) => entry.isIntersecting);
      if (visibleEntries.length > 0) {
        // Sort by intersectionRatio or boundingClientRect.top to find the one closest to the top of the viewport
        const topEntry = visibleEntries.reduce((prev, current) => {
          return Math.abs(current.boundingClientRect.top - 120) < Math.abs(prev.boundingClientRect.top - 120)
            ? current
            : prev;
        });
    ...
    observer.current = new IntersectionObserver(handleIntersection, {
      root: null,
      rootMargin: '-15% 0px -65% 0px', // focused on top-middle third of screen
  ...
  return (
    <section id="menu" className="relative w-full py-24 bg-[#0A0A0A] px-4 md:px-6">
      {/* Decorative Blur */}
      ...
      <div className="mx-auto max-w-7xl">
        ...
        <div className="flex flex-col items-center text-center mb-12 md:mb-16">
          <span className="font-sans text-xs tracking-[0.4em] text-gold uppercase font-bold mb-2">Taste the Perfection</span>
          <h2 className="font-display text-4xl md:text-6xl tracking-wider text-white uppercase">
            OUR GOURMET <span className="text-gold">MENU</span>
          </h2>
          <p className="font-serif italic text-gold-dark/80 text-sm md:text-base mt-2">
            Peruse our selection of expertly prepared recipes with authentic taste
          </p>
          <div className="h-[2px] w-20 bg-gradient-to-r from-transparent via-gold to-transparent mt-4"></div>
        </div>

        {/* Mobile Sticky Horizontal Categories Bar (KFC Style) */}
        <div className="sticky top-[86px] z-30 -mx-4 px-4 py-3 bg-[#0A0A0A]/95 backdrop-blur-md border-b border-white/5 lg:hidden overflow-x-auto flex gap-2 scrollbar-none mb-8 shadow-md">
          {MENU_CATEGORIES.map((category) => (
            <button
              key={category.id}
              onClick={() => handleCategoryClick(category.id)}
              className={`font-sans text-[10px] uppercase tracking-widest px-4 py-2.5 rounded-xl transition-all duration-300 font-semibold cursor-pointer flex-shrink-0 border ${
                activeCategory === category.id
                  ? 'bg-gold border-gold text-[#0A0A0A] font-bold shadow-md'
                  : 'text-cream/70 border-white/5 bg-[#171512] hover:text-white hover:bg-white/5'
              }`}
            >
              {category.name}
            </button>
        ...
        {/* Main Content Layout: Sidebar + Stacked Menu Grid */}
        <div className="flex flex-col lg:flex-row gap-8 items-start">
          
          {/* Desktop Sticky Sidebar Navigation (KFC Style) */}
          <aside className="w-64 flex-shrink-0 sticky top-32 hidden lg:block bg-[#12110F] border border-gold/15 p-5 rounded-2xl shadow-xl">
            <h3 className="font-display text-sm text-gold tracking-widest uppercase mb-4 border-b border-gold/10 pb-3 flex items-center gap-2">
              <span>Categories</span>
              <div className="h-[2px] flex-1 bg-gradient-to-r from-gold/20 to-transparent"></div>
            </h3>
            <div className="flex flex-col gap-1">
              {MENU_CATEGORIES.map((category) => (
                <button
                  key={category.id}
                  onClick={() => handleCategoryClick(category.id)}
                  className={`flex items-center justify-between font-sans text-xs uppercase tracking-widest px-4 py-3 rounded-xl transition-all duration-300 font-bold text-left cursor-pointer group ${
                    activeCategory === category.id
                      ? 'bg-gold text-[#0A0A0A] shadow-md scale-[1.02]'
                ...
                >
                  ...
                  <ChevronRight className={`h-3 w-3 transition-transform duration-300 ${
                    activeCategory === category.id 
                      ? 'translate-x-0 opacity-100 text-[#0A0A0A]' 
                      : 'opacity-0 -translate-x-2 group-hover:opacity-50 group-hover:translate-x-0 text-gold'
                  }`} />
                </button>
              ))}
            </div>
          ...
          {/* Vertical Stacked Food Categories Grid */}
                        ...
                        >
                          ...
                          {item.image ? (
                            <div className="relative h-56 w-full overflow-hidden bg-black/40">
                              ...
                              <div className="absolute top-4 right-4 flex items-center gap-1 bg-black/60 border border-gold/20 backdrop-blur-md px-2 py-1 rounded-md">
                                <Star className="h-2.5 w-2.5 text-gold fill-gold" />
                                <span className="font-mono text-[9px] text-gold font-bold">5.0</span>
                              </div>
                            </div>
                          ) : (
                            ...
                            <div className="px-6 pt-6 flex items-center justify-between">
                              ...
                              <div className="flex items-center gap-1 bg-black/60 border border-gold/20 backdrop-blur-md px-2 py-1 rounded-md">
                                <Star className="h-2.5 w-2.5 text-gold fill-gold" />
                                <span className="font-mono text-[9px] text-gold font-bold">5.0</span>
                              </div>
                            </div>
                          ...
                          <div className={`flex-1 flex flex-col p-6 justify-between ${item.image ? 'pt-3' : 'pt-4'}`}>
                            <div>
                              ...
                              <p className="font-sans text-xs text-cream/65 leading-relaxed tracking-wide mt-2 min-h-[44px]">
                                {item.description}
                              </p>
                                      ...
                                      <button
                                        ...
                                        className={`flex-1 flex items-center justify-center gap-1 font-sans text-[9px] tracking-wider uppercase py-1.5 rounded-lg border transition-all cursor-pointer ${
                                          sizeLabel === size
                                            ...
                                            : 'bg-white/5 text-cream/40 border-transparent hover:bg-white/10'
                                        }`}
                                      >
